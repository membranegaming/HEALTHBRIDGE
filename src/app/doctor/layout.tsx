...
export default function DoctorLayout({ children }: DoctorLayoutProps) {
  ...
  const navItems = [
    ...
    { href: "/doctor/queue", label: "Patient Queue", icon: Clock, badge: "8" },
      ...
      <aside className={`${styles.sidebar} ${sidebarOpen ? styles.sidebarOpen : ""}`}>
        <div className={styles.sidebarHeader}>
          <Link href="/" className={styles.sidebarLogo}>
            <div className={`${styles.sidebarLogoIcon} ${styles.doctor}`}>
              <Stethoscope size={24} />
            </div>
            <span>HealthBridge</span>
          </Link>
          <span className={`${styles.portalBadge} ${styles.doctor}`}>Doctor Portal</span>
        </div>
          ...
          <div className={styles.navSection}>
            ...
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`${styles.navLink} ${pathname === item.href ? `${styles.active} ${styles.doctor}` : ""}`}
                onClick={() => setSidebarOpen(false)}
              >
                <item.icon size={20} />
                <span>{item.label}</span>
                {item.badge && <span className={`${styles.navBadge} ${styles.doctor}`}>{item.badge}</span>}
              </Link>
            ))}
          </div>
        </nav>
        ...
        <div className={styles.sidebarFooter}>
          <div className={styles.sidebarUser} onClick={() => setShowUserMenu(!showUserMenu)}>
            <div className={`${styles.avatar} ${styles.avatarMd} ${styles.doctor}`}>
              <span>SJ</span>
            </div>
            <div className={styles.sidebarUserInfo}>
              <div className={styles.sidebarUserName}>Dr. Sarah Johnson</div>
              <div className={styles.sidebarUserRole}>Cardiologist</div>
            </div>
            <ChevronDown size={18} />
          </div>
          ...
          {showUserMenu && (
            <div className={styles.userDropdown}>
              <Link href="/doctor/profile" className={styles.dropdownItem}>
                <User size={18} />
                <span>Profile</span>
              </Link>
